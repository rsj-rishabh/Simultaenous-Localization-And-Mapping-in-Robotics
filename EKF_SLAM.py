...
from numpy import *
from slam_library import get_observations, write_cylinders, write_error_ellipses
...
class ExtendedKalmanFilterSLAM:
    def __init__(self, state, covariance,
        ...
        # Initially, the number of landmarks is zero.
    ...
    def predict(self, control):
        ...
        # Update only first 3 state variables i.e. robot's pose
        self.state[:3] = self.g(self.state, control, self.robot_width)
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        N = self.number_of_landmarks
        self.state = concatenate([self.state, array([x, y])])
        s = self.covariance.shape[0] # Current size of covariance matrix
        self.covariance = pad(self.covariance, (0, 2), 'constant') # Pad np array with zeros at bottom and right
        self.covariance[s:s+2, s:s+2] = diag([1e10, 1e10])
        return N-1  # Return index of landmark.
    ...
    def correct(self, measurement, landmark_index):
        ...
        H = zeros((2, 3+2*N))
        H[0:2, 0:3] = H3
        H[0:2, 3+2*landmark_index: 3+2*landmark_index+2] = H[0:2, 0:2] * -1
...
if __name__ == '__main__':
    ...
    f = open("data/ekf_slam_output.txt", "w")
